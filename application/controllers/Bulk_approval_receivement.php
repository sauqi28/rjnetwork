...
class Bulk_approval_receivement extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('penerimaan/Bulk_approval_receivement_model');
		$this->load->helper('url_helper');
		$this->load->library('form_validation');
		$this->load->library('pagination');
		$this->load->library('Wa_api');
		$this->title = "Pengajuan Approval Seluruh Penerimaan Sekaligus";
		$this->subtitle = "halaman untuk mengajukan penandatangan penerimaan sekaligus";
	...
	public function search_materials()
	{
		...
		$materials = $this->Bulk_approval_receivement_model->search_materials($search);
		$results = [];
	...
	// {
	// 	$data['users'] = $this->Bulk_approval_receivement_model->get_penerimaan_sap();
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->Bulk_approval_receivement_model->get_penerimaan_sap_count($search);
		...
		$data['users'] = $this->Bulk_approval_receivement_model->get_penerimaan_sap($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "penerimaan_sap";

		$this->load->view('data/bulk_approval_receivement/index', $data);
	}
	...
	public function non_aktif()
	{
		...
		$config['base_url'] = site_url('penerimaan_sap/index');
		$config['total_rows'] = $this->Bulk_approval_receivement_model->get_penerimaan_sap_count_nonaktif($search);
		$config['per_page'] = 30;
		$config['uri_segment'] = 3;
		$config['reuse_query_string'] = TRUE;
		...
		$data['users'] = $this->Bulk_approval_receivement_model->get_penerimaan_sap_nonaktif($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "penerimaan_sap_nonaktif";
	...
	{
		$data['user'] = $this->Bulk_approval_receivement_model->get_penerimaan_sap_view($id);

		$data['get_sign_data_tug4'] = $this->Bulk_approval_receivement_model->get_sign_data($id, 4);
		$data['get_sign_data_tug3k'] = $this->Bulk_approval_receivement_model->get_sign_data($id, 5);
		$data['get_sign_data_tug3'] = $this->Bulk_approval_receivement_model->get_sign_data($id, 6);
		//data approval berdasarkan form signature dengan id 1
		$data['tug4'] = $this->Bulk_approval_receivement_model->get_form_approval(4);
		$data['tug3k'] = $this->Bulk_approval_receivement_model->get_form_approval(5);
		$data['tug3'] = $this->Bulk_approval_receivement_model->get_form_approval(6);
		// var_dump($data);
		...
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "";
		$data['user_positions'] = $this->Bulk_approval_receivement_model->get_all_positions();
		$data['user_roles'] = $this->Bulk_approval_receivement_model->get_all_roles();
		$data['user_category'] = $this->Bulk_approval_receivement_model->get_all_category();
		$data['percentage'] = $this->Bulk_approval_receivement_model->get_percentage($id);
		$this->load->view('data/bulk_aproval_receivement/view', $data);
	}
	...
	public function create()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$data['pabrikan'] = $this->Bulk_approval_receivement_model->get_pabrikan();
			$data['material'] = $this->Bulk_approval_receivement_model->get_material();
			// $this->session->set_flashdata('message', 'Oooops!!! Something Wrong');
			// $this->session->set_flashdata('status', 'error');
			$this->load->view('data/bulk_aproval_receivement/create', $data);
		} else {
			// var_dump($this->input->post('material'));
			// exit(0);
			$this->Bulk_approval_receivement_model->create_penerimaan();
			$this->session->set_flashdata('message', 'Penerimaan SAP berhasil ditambahkan');
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('penerimaan_sap/index'));
		}
	}
	//ok
	...
	public function merge_document()
	{
		...
		$data_penerimaan = $this->Bulk_approval_receivement_model->merge_document($key);
		...
		if ($data_penerimaan) {
			...
			$output_filename = 'penerimaan_sap_' . $data_penerimaan->spk_number . '.pdf';
	...
	{
		$pdf_file_tug4 = $this->Bulk_approval_receivement_model->get_pdf_file($filename, 'tug4');
		$pdf_file_tug3k = $this->Bulk_approval_receivement_model->get_pdf_file($filename, 'tug3k');
		$pdf_file_tug3 = $this->Bulk_approval_receivement_model->get_pdf_file($filename, 'tug3');
		...
		if ($pdf_file_tug4 or $pdf_file_tug3k or $pdf_file_tug3) {
			...
			if (file_exists($pdf_file_tug4) or file_exists($pdf_file_tug3k) or file_exists($pdf_file_tug3)) {
				...
				$data['subtitle'] = $this->subtitle;
				$data['navbar'] = "";
				$this->load->view('data/bulk_aproval_receivement/view_pdf', $data);
			} else {
				show_404();
			}
		} else {
	...
	public function upload_tug3k()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Bulk_approval_receivement_model->save_file_tug3k($id, $new_name, $folder_path);
	...
	public function upload_tug3()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Bulk_approval_receivement_model->save_file_tug3($id, $new_name, $folder_path);
	...
	public function send_message()
	{
		...
		$data = $this->Bulk_approval_receivement_model->get_data_queue_sign($token);
		if ($data) {
			...
			if ($res == 'success') {
				$this->Bulk_approval_receivement_model->update_data_queue_sign($token);
				echo $res;
			} else {
				echo $res;
			}
	...
	public function send_message_bulk($token)
	{
		...
		if ($data) {
			...
			$this->session->set_flashdata('status', 'success');
			// Redirect ke halaman view
			redirect('penerimaan_sap/view/' . $token);
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Bulk_approval_receivement_model->update_user($id);
	...
	public function verify()
	{
		...
		$this->Bulk_approval_receivement_model->verify_document($key, $token);
	...
	public function update_status()
	{
		...
		if ($status == 1) {
			$this->Bulk_approval_receivement_model->update_status($id, 'inactive');
			$message = 'Berhasil dinonaktifkan';

			$this->session->set_flashdata('message', $message);
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('penerimaan_sap/index'), 'refresh');
		} elseif ($status == 2) {
			$this->Bulk_approval_receivement_model->update_status($id, 'active');
			$message = 'Berhasil diaktifkan';

			$this->session->set_flashdata('message', $message);
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('penerimaan_sap/non_aktif'), 'refresh');
		} else {
			show_404();
		}
	}
