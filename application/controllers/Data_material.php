...
class Data_material extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('data/data_material_model');
		$this->load->helper('url_helper');
		$this->load->library('form_validation');
		$this->load->library('pagination');
		$this->load->library('Wa_api');
		$this->title = "Master Data Material";
		$this->subtitle = "halaman untuk mengatur informasi, mengubah, maupun menambahkan material";
	...
	public function index()
	{
		...
		$config['base_url'] = site_url('data_material/index');
		$config['total_rows'] = $this->data_material_model->get_materials_count($search);
		$config['per_page'] = 20;
		...
		$data['materials'] = $this->data_material_model->get_materials($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "data_material";

		$this->load->view('data/material/index', $data);
	}
	...
	public function material_category()
	{
		...
		$config['base_url'] = site_url('data_material/material_category');
		$config['total_rows'] = $this->data_material_model->get_materials_count($search);
		$config['per_page'] = 10;
		$config['uri_segment'] = 3;
		$config['reuse_query_string'] = TRUE;
		...
		$data['users'] = $this->data_material_model->get_category($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "data_category";

		$this->load->view('data/material/category', $data);
	}
	...
	{
		$data['user'] = $this->data_material_model->get_material_view($id);
		// var_dump($data);
		...
		$this->load->view('data/material/view', $data);
	}
	...
	public function create()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			$data['title'] = $this->title;
			$data['subtitle'] = $this->subtitle;
			$data['navbar'] = "data_material_add";
			$this->load->view('data/material/create', $data);
		} else {
			$this->data_material_model->create_material();
			$this->session->set_flashdata('message', 'Material berhasil ditambahkan');
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('data_material/index'));
		}
	}
	...
	public function import_material()
	{
		...
		$config['base_url'] = site_url('data_material/import_material');
		$config['total_rows'] = $this->data_material_model->get_materials_count_temp($search);
		$config['per_page'] = 20;
		$config['uri_segment'] = 3;
		$config['reuse_query_string'] = TRUE;
		...
		$data['is_mst_material_temp_empty'] = $this->data_material_model->is_mst_materials_temp_empty();
		$data['materials'] = $this->data_material_model->get_materials_temp($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "data_material_import";
		$this->load->view('data/material/import', $data);
	}
	...
	public function commit_data()
	{
		...
		$this->data_material_model->copy_table();
	...
	public function reset_Data()
	{
		...
		$this->data_material_model->reset_tabel_temp();

		// Redirect ke halaman sebelumnya dengan pesan sukses
		$this->session->set_flashdata('message', 'Berhasil di Reset');
		$this->session->set_flashdata('status', 'success');
		redirect(base_url('data_material/import_material'));
	}
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->load->view('data/material/edit', $data);
		} else {
			$this->data_material_model->update_material($id);
	...
	public function import()
	{
		...
		$this->data_material_model->insert($data);
