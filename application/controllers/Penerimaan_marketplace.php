...
class Penerimaan_marketplace extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('penerimaan/Penerimaan_marketplace_model');
		$this->load->helper('url_helper');
		$this->load->library('form_validation');
		$this->load->library('pagination');
		$this->load->library('Wa_api');
		$this->title = "Penerimaan Marketplace";
		$this->subtitle = "halaman untuk informasi penerimaan marketplace TUG 4, TUG 3 Karantina & TUG 3 Persediaan";
	...
	// {
	// 	$data['users'] = $this->Penerimaan_marketplace_model->get_penerimaan_marketplace();
	...
	public function index()
	{
		...
		$config['base_url'] = site_url('penerimaan_marketplace/index');
		$config['total_rows'] = $this->Penerimaan_marketplace_model->get_penerimaan_marketplace_count($search);
		$config['per_page'] = 10;
		...
		$data['users'] = $this->Penerimaan_marketplace_model->get_penerimaan_marketplace($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "penerimaan_marketplace";

		$this->load->view('data/penerimaan_marketplace/index', $data);
	}
	...
	public function non_aktif()
	{
		...
		$config['base_url'] = site_url('penerimaan_marketplace/index');
		$config['total_rows'] = $this->Penerimaan_marketplace_model->get_penerimaan_marketplace_count_nonaktif($search);
		$config['per_page'] = 10;
		$config['uri_segment'] = 3;
		$config['reuse_query_string'] = TRUE;
		...
		$data['users'] = $this->Penerimaan_marketplace_model->get_penerimaan_marketplace_nonaktif($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "penerimaan_marketplace_nonaktif";
	...
	public function view($id = NULL)
	{
		...
		$data['get_sign_data_tug4'] = $this->Penerimaan_marketplace_model->get_sign_data($id, 1);
		$data['get_sign_data_tug3k'] = $this->Penerimaan_marketplace_model->get_sign_data($id, 2);
		$data['get_sign_data_tug3'] = $this->Penerimaan_marketplace_model->get_sign_data($id, 3);
		//data approval berdasarkan form signature dengan id 1
		$data['tug4'] = $this->Penerimaan_marketplace_model->get_form_approval(1);
		$data['tug3k'] = $this->Penerimaan_marketplace_model->get_form_approval(2);
		$data['tug3'] = $this->Penerimaan_marketplace_model->get_form_approval(3);
		// var_dump($data);
	...
	{
		$this->form_validation->set_rules('spk_number', 'spk_number', 'trim|required|is_unique[data_penerimaan_marketplace.spk_number]');
		...
		if ($this->form_validation->run() === FALSE) {
			$data['title'] = $this->title;
			$data['subtitle'] = $this->subtitle;
			$data['navbar'] = "penerimaan_marketplace_add";
			// $this->session->set_flashdata('message', 'Oooops!!! Something Wrong');
			// $this->session->set_flashdata('status', 'error');
			$this->load->view('data/penerimaan_marketplace/create', $data);
		} else {
			$this->Penerimaan_marketplace_model->create_penerimaan();
			$this->session->set_flashdata('message', 'Penerimaan Marketplace berhasil ditambahkan');
	...
	public function merge_document()
	{
		...
		$data_penerimaan = $this->Penerimaan_marketplace_model->merge_document($key);
	...
	public function view_pdf($filename)
	{
		...
		$pdf_file_tug3k = $this->Penerimaan_marketplace_model->get_pdf_file($filename, 'tug3k');
		$pdf_file_tug3 = $this->Penerimaan_marketplace_model->get_pdf_file($filename, 'tug3');
		...
		if ($pdf_file_tug4 or $pdf_file_tug3k or $pdf_file_tug3) {
			...
			if (file_exists($pdf_file_tug4) or file_exists($pdf_file_tug3k) or file_exists($pdf_file_tug3)) {
				...
				$data['subtitle'] = $this->subtitle;
				$data['navbar'] = "";
				$this->load->view('data/penerimaan_marketplace/view_pdf', $data);
			} else {
				show_404();
			}
		} else {
	...
	public function upload_tug4()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			$folder_path = './uploads/marketplace/' . str_replace('/', '-', $spk_number) . $id;
			...
			$this->Penerimaan_marketplace_model->save_file_tug4($id, $new_name, $folder_path);

			// Set pesan sukses sebagai flashdata
			$this->session->set_flashdata('message', 'TUG 4 Marketplace berhasil diupload');
			$this->session->set_flashdata('status', 'success');
			// Redirect ke halaman view
			redirect('penerimaan_marketplace/view/' . $id);
		} else {
	...
	public function upload_tug3k()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Penerimaan_marketplace_model->save_file_tug3k($id, $new_name, $folder_path);

			// Set pesan sukses sebagai flashdata
			$this->session->set_flashdata('message', 'TUG 3 Karantina Marketplace berhasil diupload');
			$this->session->set_flashdata('status', 'success');
			// Redirect ke halaman view
			redirect('penerimaan_marketplace/view/' . $id);
		} else {
	...
	public function upload_tug3()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Penerimaan_marketplace_model->save_file_tug3($id, $new_name, $folder_path);

			// Set pesan sukses sebagai flashdata
			$this->session->set_flashdata('message', 'TUG 3 Persediaan Marketplace berhasil diupload');
			$this->session->set_flashdata('status', 'success');
			// Redirect ke halaman view
			redirect('penerimaan_marketplace/view/' . $id);
		} else {
	...
	public function send_message()
	{
		...
		if ($data) {
			...
			if ($res == 'success') {
				$this->Penerimaan_marketplace_model->update_data_queue_sign($token);
				echo $res;
			} else {
				echo $res;
			}
	...
	public function send_message_bulk($token)
	{
		...
		if ($data) {
			...
			$this->session->set_flashdata('status', 'success');
			// Redirect ke halaman view
			redirect('penerimaan_marketplace/view/' . $token);
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Penerimaan_marketplace_model->update_user($id);
	...
	public function verify()
	{
		...
		$this->Penerimaan_marketplace_model->verify_document($key, $token);
	...
	public function update_status()
	{
		...
		if ($status == 1) {
			$this->Penerimaan_marketplace_model->update_status($id, 'inactive');
			$message = 'Berhasil dinonaktifkan';

			$this->session->set_flashdata('message', $message);
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('penerimaan_marketplace/index'), 'refresh');
		} elseif ($status == 2) {
			$this->Penerimaan_marketplace_model->update_status($id, 'active');
			$message = 'Berhasil diaktifkan';

			$this->session->set_flashdata('message', $message);
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('penerimaan_marketplace/non_aktif'), 'refresh');
		} else {
			show_404();
		}
	}
