...
class Penerimaan_sap extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('penerimaan/Penerimaan_sap_model');
		$this->load->helper('url_helper');
		$this->load->library('form_validation');
		$this->load->library('pagination');
		$this->load->library('Wa_api');
		$this->title = "Penerimaan SAP Pengadaan";
	...
	public function index()
	{
		...
		$data['users'] = $this->Penerimaan_sap_model->get_penerimaan_sap($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "penerimaan_sap";

		$this->load->view('data/penerimaan_sap/index', $data);
	}
	...
	public function non_aktif()
	{
		...
		$data['users'] = $this->Penerimaan_sap_model->get_penerimaan_sap_nonaktif($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
	...
	{
		$data['user'] = $this->Penerimaan_sap_model->get_penerimaan_sap_view($id);

		$data['get_sign_data_tug4'] = $this->Penerimaan_sap_model->get_sign_data($id, 4);
		$data['get_sign_data_tug3k'] = $this->Penerimaan_sap_model->get_sign_data($id, 5);
		$data['get_sign_data_tug3'] = $this->Penerimaan_sap_model->get_sign_data($id, 6);
		//data approval berdasarkan form signature dengan id 1
		$data['tug4'] = $this->Penerimaan_sap_model->get_form_approval(4);
		$data['tug3k'] = $this->Penerimaan_sap_model->get_form_approval(5);
		$data['tug3'] = $this->Penerimaan_sap_model->get_form_approval(6);
		// var_dump($data);
		...
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "";
		$data['user_positions'] = $this->Penerimaan_sap_model->get_all_positions();
		$data['user_roles'] = $this->Penerimaan_sap_model->get_all_roles();
		$data['user_category'] = $this->Penerimaan_sap_model->get_all_category();
		$data['percentage'] = $this->Penerimaan_sap_model->get_percentage($id);
		$this->load->view('data/penerimaan_sap/view', $data);
	}
	...
	public function merge_document()
	{
		...
		$data_penerimaan = $this->Penerimaan_sap_model->merge_document($key);
	...
	{
		$pdf_file_tug4 = $this->Penerimaan_sap_model->get_pdf_file($filename, 'tug4');
		$pdf_file_tug3k = $this->Penerimaan_sap_model->get_pdf_file($filename, 'tug3k');
		$pdf_file_tug3 = $this->Penerimaan_sap_model->get_pdf_file($filename, 'tug3');
		...
		if ($pdf_file_tug4 or $pdf_file_tug3k or $pdf_file_tug3) {
			...
			if (file_exists($pdf_file_tug4) or file_exists($pdf_file_tug3k) or file_exists($pdf_file_tug3)) {
				...
				$data['subtitle'] = $this->subtitle;
				$data['navbar'] = "";
				$this->load->view('data/penerimaan_sap/view_pdf', $data);
			} else {
				show_404();
			}
		} else {
	...
	public function upload_tug4()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			$this->Penerimaan_sap_model->save_file_tug4($id, $new_name, $folder_path);
	...
	public function upload_tug3k()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Penerimaan_sap_model->save_file_tug3k($id, $new_name, $folder_path);
	...
	public function upload_tug3()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Penerimaan_sap_model->save_file_tug3($id, $new_name, $folder_path);
	...
	public function send_message()
	{
		...
		$data = $this->Penerimaan_sap_model->get_data_queue_sign($token);
		if ($data) {
			...
			if ($res == 'success') {
				$this->Penerimaan_sap_model->update_data_queue_sign($token);
				echo $res;
			} else {
				echo $res;
			}
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Penerimaan_sap_model->update_user($id);
	...
	public function verify()
	{
		...
		$this->Penerimaan_sap_model->verify_document($key, $token);
	...
	public function sign_document()
	{
		...
		$this->Penerimaan_sap_model->process_sign($key, $this->generate_token(20), 4, "TUG 4 (Berita Acara) Penerimaan SAP");
		$this->Penerimaan_sap_model->process_sign($key, $this->generate_token(20), 5, "TUG 3 (Karantina) Penerimaan SAP");
		$this->Penerimaan_sap_model->process_sign($key, $this->generate_token(20), 6, "TUG 3 (Persediaan) Penerimaan SAP");
	...
	public function update_status()
	{
		...
		if ($status == 1) {
			$this->Penerimaan_sap_model->update_status($id, 'inactive');
			...
			$this->Penerimaan_sap_model->update_status($id, 'active');
