...
class Penerimaan_sap_return extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('penerimaan/Penerimaan_sap_return_model');
		$this->load->helper('url_helper');
		$this->load->library('form_validation');
		$this->load->library('pagination');
		$this->load->library('Wa_api');
		$this->title = "Penerimaan SAP Retur Pekerjaan";
	...
	// {
	// 	$data['users'] = $this->Penerimaan_sap_return_model->get_Penerimaan_sap_return();
	...
	public function index()
	{
		...
		$data['users'] = $this->Penerimaan_sap_return_model->get_Penerimaan_sap_return($config['per_page'], $page, $search);
	...
	public function non_aktif()
	{
		...
		$data['users'] = $this->Penerimaan_sap_return_model->get_Penerimaan_sap_return_nonaktif($config['per_page'], $page, $search);
		$data['pagination'] = $this->pagination->create_links();
		$data['title'] = $this->title;
		$data['subtitle'] = $this->subtitle;
	...
	{
		$data['user'] = $this->Penerimaan_sap_return_model->get_Penerimaan_sap_return_view($id);

		$data['get_sign_data_tug4'] = $this->Penerimaan_sap_return_model->get_sign_data($id, 7);
		$data['get_sign_data_tug3k'] = $this->Penerimaan_sap_return_model->get_sign_data($id, 8);
		$data['get_sign_data_tug3'] = $this->Penerimaan_sap_return_model->get_sign_data($id, 9);
		//data approval berdasarkan form signature dengan id 1
		$data['tug4'] = $this->Penerimaan_sap_return_model->get_form_approval(7);
		$data['tug3k'] = $this->Penerimaan_sap_return_model->get_form_approval(8);
		$data['tug3'] = $this->Penerimaan_sap_return_model->get_form_approval(9);
		// var_dump($data);
		...
		$data['subtitle'] = $this->subtitle;
		$data['navbar'] = "";
		$data['user_positions'] = $this->Penerimaan_sap_return_model->get_all_positions();
	...
	public function merge_document()
	{
		...
		$data_penerimaan = $this->Penerimaan_sap_return_model->merge_document($key);
		...
		if ($data_penerimaan) {
			...
			$output_filename = 'Penerimaan_sap_return_' . $data_penerimaan->spk_number . '.pdf';
	...
	{
		$pdf_file_tug4 = $this->Penerimaan_sap_return_model->get_pdf_file($filename, 'tug4');
		$pdf_file_tug3k = $this->Penerimaan_sap_return_model->get_pdf_file($filename, 'tug3k');
		$pdf_file_tug3 = $this->Penerimaan_sap_return_model->get_pdf_file($filename, 'tug3');
	...
	public function upload_tug4()
	{
		...
		$config['upload_path'] = './uploads/sapreturn/';
		...
		if ($this->upload->do_upload('file')) {
			...
			$folder_path = './uploads/sapreturn/' . str_replace('/', '-', $spk_number) . $id;
			...
			$this->Penerimaan_sap_return_model->save_file_tug4($id, $new_name, $folder_path);
	...
	public function upload_tug3k()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Penerimaan_sap_return_model->save_file_tug3k($id, $new_name, $folder_path);
	...
	public function upload_tug3()
	{
		...
		if ($this->upload->do_upload('file')) {
			...
			// Simpan nama file ke database
			$this->Penerimaan_sap_return_model->save_file_tug3($id, $new_name, $folder_path);
	...
	public function send_message()
	{
		...
		$data = $this->Penerimaan_sap_return_model->get_data_queue_sign($token);
		if ($data) {
			...
			if ($res == 'success') {
				$this->Penerimaan_sap_return_model->update_data_queue_sign($token);
				echo $res;
			} else {
				echo $res;
			}
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Penerimaan_sap_return_model->update_user($id);
	...
	public function verify()
	{
		...
		$this->Penerimaan_sap_return_model->verify_document($key, $token);
	...
	public function update_status()
	{
		...
		if ($status == 1) {
			$this->Penerimaan_sap_return_model->update_status($id, 'inactive');
			$message = 'Berhasil dinonaktifkan';

			$this->session->set_flashdata('message', $message);
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('Penerimaan_sap_return/index'), 'refresh');
		} elseif ($status == 2) {
			$this->Penerimaan_sap_return_model->update_status($id, 'active');
			$message = 'Berhasil diaktifkan';

			$this->session->set_flashdata('message', $message);
			$this->session->set_flashdata('status', 'success');
			redirect(base_url('Penerimaan_sap_return/non_aktif'), 'refresh');
		} else {
			show_404();
		}
	}
