...
class Signsap2receivement extends CI_Controller
{
  public function __construct()
  {
    parent::__construct();
    $this->load->model('Sign_sapreceivement_return_model');
  ...
  public function return()
  {
    ...
    $user_data = $this->Sign_sapreceivement_return_model->get_data_by_token($token);
    ...
    if ($user_data && $this->_is_token_valid($user_data->request_at)) {
      ...
      $this->load->view('digitalsign/publicsapreturn', $user_data);
    } else {
  ...
  public function single_approve()
  {
    ...
    if ($data->form_id == 7) {
      ...
      $result = $this->Sign_sapreceivement_return_model->approve_signature($id, $token, $data->form_id);
      $approval_sum = $this->Sign_sapreceivement_return_model->update_current_count_sign($token);

      //kirim pesan ke manajer
      //if ($approval_sum >= 5 && $approval_sum <= 7 && $approval_sum != 8) {
      if ($approval_sum == 7 && $approval_sum != 8) {
        $send_manager = $this->Sign_sapreceivement_return_model->send_manager($token);
      }
    ...
    } else if ($data->form_id == 9) { // tug 3 persediaan
      // Melakukan update pada model Approval_single
      $result = $this->Sign_sapreceivement_return_model->approve_signature($id, $token, $data->form_id);
      $approval_sum = $this->Sign_sapreceivement_return_model->update_current_count_sign($token);
  ...
  //   if ($data->form_id == 1) {
  //     // Melakukan update pada model Approval_single
  //     $result = $this->Sign_sapreceivement_return_model->approve_signature($id, $token);
  //     $approval_sum = $this->Sign_sapreceivement_return_model->update_current_count_sign($token);
  ...
  public function stamp_sign($token)
  {
    try {
      ...
      for ($i = 1; $i <= $page_count; $i++) {
        ...
        if ($data->form_id == 7 && $data->sequence < 8) {
        ...
        } else if ($data->form_id == 7 && $data->sequence == 8) { //manager
  ...
  public function stamp_sign_tug3_persediaan($token)
  {
    try {
      ...
      for ($i = 1; $i <= $page_count; $i++) {
        ...
        if ($data->form_id == 9 && $data->sequence < 8) {
