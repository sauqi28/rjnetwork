...
class Signsap3receivement extends CI_Controller
{
  public function __construct()
  {
    parent::__construct();
    $this->load->model('Sign_sapreceivement_intracompany_model');
    $this->load->helper('url');
  }

  public function intracompany()
  {
    ...
    if ($user_data && $this->_is_token_valid($user_data->request_at)) {
      ...
      $this->load->view('digitalsign/publicsapintracompany', $user_data);
    } else {
  ...
  public function single_approve()
  {
    ...
    $data = $this->Sign_sapreceivement_intracompany_model->get_data_by_token($token);

    //formulir untuk tug 4 marketplace
    if ($data->form_id == 10) {
      // Melakukan update pada model Approval_single
      $result = $this->Sign_sapreceivement_intracompany_model->approve_signature($id, $token, $data->form_id);
      $approval_sum = $this->Sign_sapreceivement_intracompany_model->update_current_count_sign($token);

      //kirim pesan ke manajer
      //if ($approval_sum >= 5 && $approval_sum <= 7 && $approval_sum != 8) {
      if ($approval_sum == 7 && $approval_sum != 8) {
        $send_manager = $this->Sign_sapreceivement_intracompany_model->send_manager($token);
      }
    ...
    } else if ($data->form_id == 12) { // tug 3 persediaan
      // Melakukan update pada model Approval_single
      $result = $this->Sign_sapreceivement_intracompany_model->approve_signature($id, $token, $data->form_id);
      $approval_sum = $this->Sign_sapreceivement_intracompany_model->update_current_count_sign($token);
  ...
  //   if ($data->form_id == 1) {
  //     // Melakukan update pada model Approval_single
  //     $result = $this->Sign_sapreceivement_intracompany_model->approve_signature($id, $token);
  //     $approval_sum = $this->Sign_sapreceivement_intracompany_model->update_current_count_sign($token);
  ...
  public function stamp_sign($token)
  {
    try {
      ...
      for ($i = 1; $i <= $page_count; $i++) {
        ...
        if ($data->form_id == 10 && $data->sequence < 8) {
        ...
        } else if ($data->form_id == 10 && $data->sequence == 8) { //manager
          $image_width = (5 * 5.4); // Width in points (1 point = 1/72 inches)
          $image_height = (3 * 5.4); // Height in points
          $position_x = $data->x_sign; // X position in points
          $position_y = $data->y_sign; // Y position in points
  ...
  public function stamp_sign_tug3_persediaan($token)
  {
    try {
      ...
      for ($i = 1; $i <= $page_count; $i++) {
        ...
        if ($data->form_id == 12 && $data->sequence < 8) {
