...
class Sign_sapreceivement_intracompany_model extends CI_Model
{
  ...
  public function get_data_by_token($token)
  {
    ...
    $this->db->join('data_penerimaan_sap_intracompany d', 'c.key_reference = d.key');
  ...
  function approve_signature($id_sign, $token, $id_form)
  {
      ...
      if ($query->num_rows() > 0 && $query->row()->sequence == 8) { //jika manajer
        ...
        if ($query->num_rows() > 0) {
          foreach ($query->result() as $row) {
            ...
            $this->db->where('form_id', 12); // form tug_3 persediaan_ bast marketplace
    ...
    } else if ($id_form == 11) { // jika form tug3 karantina
      $this->db->set('approved', 1);
      $this->db->set('approved_time', $approved_time);
      $this->db->where('id', $id_sign);
      $this->db->where('token', $token);
      ...
      if ($query->num_rows() > 0) {
        foreach ($query->result() as $row) {
          ...
          if ($row->sequence == 2) {
            ...
            $this->db->from('data_process_sign');
            $this->db->where('key_reference', $key_reference);
            $this->db->where('form_id', 10); // form tug_4_ bast marketplace
            $query_id_proc = $this->db->get();
      ...
      if ($query->num_rows() > 0) {
        foreach ($query->result() as $row) {
          ...
          if ($row->sequence == 2) {
            ...
            $this->db->from('data_process_sign');
            $this->db->where('key_reference', $key_reference);
            $this->db->where('form_id', 12); // form tug_3 persediaan
            $query_id_proc = $this->db->get();
