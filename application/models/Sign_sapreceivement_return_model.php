...
class Sign_sapreceivement_return_model extends CI_Model
{
  ...
  public function get_data_by_token($token)
  {
    ...
    $this->db->join('data_penerimaan_sap_return d', 'c.key_reference = d.key');
      ...
      if ($query->num_rows() > 0 && $query->row()->sequence == 8) { //jika manajer
        ...
        if ($query->num_rows() > 0) {
          foreach ($query->result() as $row) {
            ...
            $this->db->where('form_id', 9); // form tug_3 persediaan_ bast marketplace
      ...
      if ($query->num_rows() > 0) {
        foreach ($query->result() as $row) {
          ...
          if ($row->sequence == 3) {
            ...
            $this->db->join('data_queue_sign a', 'a.id_process_reference = b.id_process');
            $this->db->where('a.token', $token);
            $this->db->where('a.sequence', 3); // penandatanganan ke 2,
            $query_key_ref = $this->db->get();
            ...
            $this->db->from('data_process_sign');
            $this->db->where('key_reference', $key_reference);
            $this->db->where('form_id', 7); // form tug_4_ bast marketplace
            $query_id_proc = $this->db->get();
      ...
      if ($query->num_rows() > 0) {
        foreach ($query->result() as $row) {
          ...
          if ($row->sequence == 2) {
            ...
            $this->db->from('data_process_sign');
            $this->db->where('key_reference', $key_reference);
            $this->db->where('form_id', 9); // form tug_3 persediaan
            $query_id_proc = $this->db->get();
