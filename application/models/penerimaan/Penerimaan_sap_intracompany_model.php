...
class Penerimaan_sap_intracompany_model extends CI_Model
{
  ...
  public function get_penerimaan_sap_intracompany_view($id = NULL)
  {
    if ($id === NULL) {
      $query = $this->db->get('data_penerimaan_sap_intracompany');
    ...
    $this->db->from('data_penerimaan_sap_intracompany a');
  ...
  public function get_penerimaan_sap_intracompany($limit, $start, $search = NULL)
  {
  ...
  public function get_pdf_file($filename, $type)
  {
    ...
    $this->db->from('data_penerimaan_sap_intracompany');
  ...
  public function get_penerimaan_sap_intracompany_count($search)
  {
    $this->db->select('a.*');
    $this->db->from('data_penerimaan_sap_intracompany a');
    if ($search) {
  ...
  public function get_penerimaan_sap_intracompany_count_nonaktif($search)
  {
    $this->db->select('a.*');
  ...
  public function create_penerimaan()
  {
    ...
    return $this->db->insert('data_penerimaan_sap_intracompany', $data);
  }
  ...
  public function save_file_tug4($id, $filename, $folder_path)
  {
    ...
    $this->db->update('data_penerimaan_sap_intracompany', $data);
  }
  ...
  public function process_sign($key, $token, $formid, $note)
  {
    ...
    for ($i = 1; $i <= $sign_position['sign_count']; $i++) {
      ...
      $request_status = 0;
      if ($formid == 11 && $i < 2) { //proses silkulir
        $request_status = 1;
      }
      ...
      $data = array(
        ...
        'uri' => '/signsap3receivement/intracompany/',
        'title' => $note,
