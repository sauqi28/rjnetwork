...
class Penerimaan_sap_return_model extends CI_Model
{
  ...
  public function get_penerimaan_sap_return_view($id = NULL)
  {
    if ($id === NULL) {
      $query = $this->db->get('data_penerimaan_sap_return');
    ...
    $this->db->from('data_penerimaan_sap_return a');
  ...
  public function get_penerimaan_sap_return($limit, $start, $search = NULL)
  {
  ...
  public function get_pdf_file($filename, $type)
  {
    ...
    $this->db->from('data_penerimaan_sap_return');
  ...
  public function get_penerimaan_sap_return_count($search)
  {
    $this->db->select('a.*');
    $this->db->from('data_penerimaan_sap_return a');
    if ($search) {
  ...
  public function get_penerimaan_sap_return_count_nonaktif($search)
  {
    $this->db->select('a.*');
  ...
  public function create_penerimaan()
  {
    ...
    return $this->db->insert('data_penerimaan_sap_return', $data);
  }
  ...
  public function save_file_tug4($id, $filename, $folder_path)
  {
    ...
    $this->db->update('data_penerimaan_sap_return', $data);
  }
  ...
  public function process_sign($key, $token, $formid, $note)
  {
    ...
    for ($i = 1; $i <= $sign_position['sign_count']; $i++) {
      ...
      $request_status = 0;
      if ($formid == 8 && $i < 2) { //proses silkulir
        $request_status = 1;
      }
      ...
      $data = array(
        ...
        'uri' => '/signsap2receivement/return/',
        'title' => $note,
